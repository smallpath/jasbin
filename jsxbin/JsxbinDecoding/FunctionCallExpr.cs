...
namespace jsxbin_to_jsx.JsxbinDecoding
{
    public class FunctionCallExpr : AbstractNode
    {
        ...
        public override string PrettyPrint()
        {
            ...
            string functionBody = functionName.PrettyPrint();
            string callExpr;
            if (functionBody.IndexOf("{") == -1)
            {
                callExpr = string.Format("{0}{1}({2})",
                                                ctor, functionName.PrettyPrint(),
                                                args == null ? "" : string.Join(", ", args.Arguments.Select(a => a.PrettyPrint())));
            }
            else
            {
                callExpr = string.Format("{0}({1})({2})",
